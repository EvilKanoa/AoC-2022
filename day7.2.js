...
const findSmallestAbove = (sizes = [], above = 0) => {
  let smallest = Number.MAX_VALUE;
  for (const size of sizes) {
    if (size >= above && size < smallest) {
      smallest = size;
    }
  }
  return smallest;
...
benchmark(() => {
  ...
  const neededSpace = 30000000 - (70000000 - files.size);

  result = findSmallestAbove(sizes, neededSpace);
});
