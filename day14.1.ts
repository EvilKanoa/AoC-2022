...
const isMoveValid = (cave: CaveState, move: Position) => {
  ...
  return !cave.rocks.has(str) && !cave.sand.has(str);
};
...
const printCave = (caveState: CaveState): string => {
  ...
  for (const rock of caveState.rocks.values()) {
    const pos = parsePosition(rock);
  ...
  for (let y = 0; y <= caveState.bottom; y++) {
    ...
    for (let x = minX - 1; x <= maxX + 1; x++) {
      ...
      if (pos === "500,0") {
        cave += "+";
      } else if (caveState.rocks.has(pos)) {
        cave += "#";
